...
function relayPlayers(room) {
  if (room.relay && Array.isArray(room.relay.playerIds)) {
    return room.relay.playerIds.map((id) => room.players.get(id)).filter(Boolean);
  }
  return [...room.players.values()].filter((player) => player.online);
}

function relayBookIndexForPlayer(room, playerIndex, step) {
  const n = relayPlayers(room).length;
  return (playerIndex + step - 1) % n;
}

function startRelayGame(room) {
  const players = relayPlayers(room);
  const pool = buildWordPool(room);
  ...
  room.relay = {
    ...
    books: players.map((p, i) => {
      const item = picked[i] || { word: '苹果', category: '食物', hint: '' };
      return {
        id: i,
  ...
  for (const p of players) {
    room.relayDrawings.set(p.id, { bookIndex: 0, events: [], activeStrokes: new Set(), lastDrawAt: 0 });
  }
...
function beginRelayStep(room, step) {
  ...
  if (step % 2 === 1) {
    room.phase = 'relay_draw';
    room.deadline = Date.now() + room.config.drawSeconds * 1000;
    players.forEach((player, playerIndex) => {
      const bookIndex = relayBookIndexForPlayer(room, playerIndex, step);
      const book = room.relay.books[bookIndex];
      const drawState = room.relayDrawings.get(player.id) || { events: [], activeStrokes: new Set(), lastDrawAt: 0 };
      drawState.bookIndex = bookIndex;
      drawState.events = [];
      drawState.activeStrokes = new Set();
      // 每一棒都必须重新收集缩略图，否则上一棒的缩略图会让上传逻辑误判为已完成。
      drawState.thumbnail = null;
      room.relayDrawings.set(player.id, drawState);
      const task = { type: 'relay_task', step, kind: 'draw', bookIndex, deadline: room.deadline };
      if (step === 1) {
        task.promptType = 'word';
        task.promptWord = book.originalWord;
        task.promptCategory = book.originalCategory;
      } else {
        const last = book.entries[book.entries.length - 1];
        task.promptType = 'guess';
        task.promptGuess = last ? (last.text || '没猜出来') : '没猜出来';
      }
      sendJson(player.ws, task);
    });
    phaseChanged(room);
    systemChat(room, `第 ${step}/${room.relay.totalSteps} 棒：每个人正在秘密作画`);
  } else {
    room.phase = 'relay_guess';
    room.deadline = Date.now() + room.config.guessSeconds * 1000;
    players.forEach((player, playerIndex) => {
      const bookIndex = relayBookIndexForPlayer(room, playerIndex, step);
      const book = room.relay.books[bookIndex];
      const last = book.entries[book.entries.length - 1];
      sendJson(player.ws, {
        type: 'relay_task', step, kind: 'guess', bookIndex, deadline: room.deadline,
        events: last ? last.events || [] : [],
        placeholder: last ? '' : '上一棒没有留下画作',
      });
    });
    phaseChanged(room);
    systemChat(room, `第 ${step}/${room.relay.totalSteps} 棒：根据上一幅画猜词`);
  }
...
function broadcastRelayProgress(room) {
  const players = relayPlayers(room);
  let done = 0;
  if (room.phase === 'relay_guess') done = room.relay.guesses.size;
  else if (room.phase === 'relay_collect') {
    done = [...room.relayDrawings.values()].filter((d) => d.thumbnail).length;
  }
  broadcast(room, {
    type: 'relay_progress',
...
function finalizeRelayStep(room) {
  if (room.phase === 'relay_draw') {
    for (const player of relayPlayers(room)) {
      const drawState = room.relayDrawings.get(player.id);
      if (!drawState) continue;
      room.relay.books[drawState.bookIndex].entries.push({
        type: 'draw',
        playerId: player.id,
  ...
  if (room.phase === 'relay_guess') {
    const players = relayPlayers(room);
    players.forEach((player, playerIndex) => {
      if (room.relay.guesses.has(player.id)) return;
      const bookIndex = relayBookIndexForPlayer(room, playerIndex, room.relay.step);
      room.relay.books[bookIndex].entries.push({
        type: 'guess',
        playerId: player.id,
        playerName: player.nickname,
        text: '没猜出来',
...
function handleRelayUpload(room, player, msg) {
  ...
  if (!image.startsWith('data:image/') || !image.includes(';base64,') || image.length > 350000) return;
...
function handleRelayGuess(room, player, msg) {
  ...
  const playerIndex = relayPlayers(room).findIndex((p) => p.id === player.id);
  if (playerIndex < 0 || room.relay.guesses.has(player.id)) return;
...
function handleRelayRate(room, player, msg) {
  ...
  for (const value of book.ratings.values()) {
    if (value === 'veg') vegCount += 1;
    else if (value === 'flower') flowerCount += 1;
  }
  broadcast(room, {
    type: 'relay_rating',
    bookIndex: book.id,
...
function revealRelay(room) {
  ...
  const books = room.relay.books.map((book) => ({
    id: book.id,
  ...
  broadcast(room, { type: 'relay_reveal', books });
  phaseChanged(room);
...
function finishGame(room) {
  ...
  room.pauseDeadline = null;
  room.paused = false;
  if (room.config.gameMode === 'relay') syncRelayArtworksToGallery(room);
...
function handleCorrectGuess(room, player) {
  ...
  const guessers = [...room.players.values()].filter((p) => p.id !== room.currentDrawerId);
...
function handleDraw(room, player, msg) {
  if (room.phase !== 'drawing' || room.paused || player.id !== room.currentDrawerId) {
    sendJson(player.ws, { type: 'error', code: 'not_drawer', message: '当前不是你的作画时间' });
    return;
  }
  if (msg.type === 'draw_begin') {
    const s = msg.stroke || {};
...
function handleUploadArtwork(room, player, msg) {
  ...
  if (!artwork || artwork.uploaderId !== player.id) {
    sendJson(player.ws, { type: 'error', code: 'not_uploader', message: '当前不是缩略图上传者' });
    return;
  }
  if (artwork.thumbnail) return;
  const image = String(msg.image || '');
  if (!/^data:image\/(jpeg|png|webp);base64,/.test(image) || image.length > 350000) {
    sendJson(player.ws, { type: 'error', code: 'bad_image', message: '缩略图格式不正确或过大' });
    return;
  }
...
function attachSocket(room, player, ws) {
  ...
  if (room.relay && (room.phase === 'relay_draw' || room.phase === 'relay_guess')) {
    ...
    if (playerIndex >= 0) {
      ...
      if (room.phase === 'relay_draw') {
        const drawState = room.relayDrawings.get(player.id);
        const task = { type: 'relay_task', step, kind: 'draw', bookIndex, deadline: room.deadline, events: drawState ? drawState.events : [] };
        if (step === 1) {
          task.promptType = 'word';
          task.promptWord = book.originalWord;
          task.promptCategory = book.originalCategory;
    ...
    sendJson(ws, { type: 'relay_reveal', books });
  }
  ...
  if (roomMatch) {
    ...
    if (pathname === `/api/rooms/${code}/join` && req.method === 'POST') {
      ...
      if (existing && !existing.online && requestedPlayerId === existing.id && requestedToken === existing.token) {
        ...
        // 传画接龙的链条在开局时按参与者固定；开始后禁止新玩家加入，避免人数变化导致链索引越界。
        if (room.phase !== 'waiting' && room.phase !== 'finished' && room.config.gameMode === 'relay') {
          sendJsonResponse(res, 409, { error: '传画接龙已经开始，暂时不能加入' });
          return;
        }
      ...
      if (rejoined) {
        ...
        if (room.phase !== 'waiting' && room.phase !== 'finished' && room.config.gameMode !== 'relay') {
          addMidgamePlayerToRotation(room, player);
        }
        broadcastPlayers(room);
