...
发布版安装包会自带 Node.js、项目依赖以及点歌所需的 `yt-dlp` / `ffmpeg` 工具，不需要用户另外安装 Node.js、执行 `npm install` 或配置 PATH。安装后从开始菜单或桌面快捷方式打开“你画我猜服务启动器”即可。

发布版的日志和点歌缓存写入当前用户的 `%LOCALAPPDATA%\速成你画我猜`，卸载程序不会删除这些运行数据。
...
项目代码使用 MIT License，完整文本见 LICENSE 文件。MIT 允许使用、修改、复制、再分发和商业使用，但需要保留版权声明和许可证文本。

第三方 MEME、音效、角色和品牌素材不自动包含在这份代码许可证内，可能受各自版权、商标或来源站点条款约束；再分发这些素材前，请按来源页面确认许可范围。
