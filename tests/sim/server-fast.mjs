// AUTO-GENERATED by tests/sim/make-fast-server.mjs — 只缩放计时/限流常量，逻辑与 server.js 一致
...
const WORD_CHOOSE_MS = 1000;
const RESULT_MS = 800;
const PERFECT_RESULT_MS = 1200;
const DRAWER_PAUSE_MS = 2000;
...
const RELAY_INTRO_MS = 300;
const RELAY_COLLECT_MS = 400;
const RELAY_REVEAL_MS = 9000;
    ...
    switch (msg.type) {
      ...
      case 'update_config':
        if (player.id === room.hostId && room.phase === 'waiting') {
          ...
          const drawSeconds = Math.min(180, Math.max(5, Number(msg.drawSeconds) || room.config.drawSeconds));
          ...
          if (msg.guessSeconds !== undefined) {
            room.config.guessSeconds = Math.min(90, Math.max(5, Number(msg.guessSeconds) || room.config.guessSeconds));
          }
          for (const p of room.players.values()) {
